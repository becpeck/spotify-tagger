...
import { AlbumIdSchema, AlbumTypeSchema, AlbumURISchema, ArtistIdSchema, ArtistTypeSchema, ArtistURISchema, PlaylistIdSchema, PlaylistTypeSchema, PlaylistURISchema, TrackIdSchema, TrackURISchema } from "@/server/spotifyWebApi/utils/schemas";
  ...
  playWithContext: protectedProcedure
    ...
    .mutation(({ input, ctx }) => {
      ...
      return ctx.spotify.startPlayback({
        context_uri: "uri" in context ? context.uri : `spotify:${context.type}:${context.id}`,
        ...(offset 
          ? ("uri" in offset 
